"""M2.1 validation: mem0 reference overlay end-to-end.

Exercises the M1 substrate against real overlay content from
config/overlays/mem0/. Proves capture -> activate -> verify
for a non-trivial, multi-file, multi-format overlay.
"""
...
OVERLAY_SOURCE = Path(__file__).resolve().parent.parent.parent / "config" / "overlays" / "mem0"
...
def test_mem0_overlay_source_exists_and_has_expected_files() -> None:
...
def test_mem0_overlay_capture_and_activate_roundtrip(tmp_path: Path) -> None:
    local_store = _init_bare_git_repo(tmp_path / "local.git")
    workspace = tmp_path / "workspace"
    workspace.mkdir()

    overlay_ref = OverlayRef(author="synapt", name="mem0")
    ...
    result = activate_overlay(
        ...
        overlay_source_value="synapt/mem0",
        overlay_signer=None,
    )
...
def test_mem0_overlay_content_fidelity_across_push_fetch(tmp_path: Path) -> None:
    local_store = _init_bare_git_repo(tmp_path / "local.git")
...
def test_mem0_overlay_deactivate_restores_clean_workspace(tmp_path: Path) -> None:
...
def test_mem0_overlay_settings_toml_has_required_fields(tmp_path: Path) -> None:
    import tomllib
    ...
    assert data["overlay"]["name"] == "mem0"
    assert data["overlay"]["repo"] == "mem0ai/mem0"
    assert data["analysis"]["benchmark"]["dataset"] == "LOCOMO"
    assert data["analysis"]["benchmark"]["baseline_score"] == 66.88
    assert "zep" in data["comparison"]["competitors"]
    assert "hindsight" in data["comparison"]["competitors"]


def test_mem0_overlay_analysis_yml_has_module_entries() -> None:
    import yaml
    ...
    assert "mem0/memory/main.py" in paths
    assert "mem0/configs/base.py" in paths

    eval_cfg = data["evaluation"]["locomo"]
    assert eval_cfg["conversations"] == 10
    assert eval_cfg["gate_conversation"] == 3
